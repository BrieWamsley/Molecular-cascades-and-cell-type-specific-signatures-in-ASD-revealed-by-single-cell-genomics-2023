...
# Change from previous version (022321)
# Remove clinical from bootstrapping
# ASD with DUP15q vs CTL
...
# get file names for seurat object by cluster 
...
for(k in 1:length(f)){
  ...
  # generate pseudoBulk count table with gene x sample
  dim(s1@assays$RNA)
  ...
  # Assinging color for each covariate
  ...
  # Correlation analysis Covariates vs Top exp PCs
  ...
  #### Calculate avg beta by bootstrapping #################
  ...
  varnames <- c("group","Chemistry10X","rnaBatch","BrainRegion","nCells","medPercMito")
  ...
  ### DEA using limma VST
  # keep CTL and ASD with DUP15 and age/sex matching CTL
  keep=c("3935","5983","14762","15451","4337","5391","5670","5842","5983","7444","EUFW","HQ4T","PZ65","UPIT3152")
  conds=conds[conds$individualID%in%keep,]
...
for(i in 1:length(f2)){
  dat.temp<-read.csv(paste0("./",f2[i]),head=T,row.names=1,stringsAsFactors = F)
